...
\newpage
    ...
    \begin{definition}\label{def:multisetOperation}
        ...
        We define the \emph{union},\emph{intersection},\emph{difference} as following, using the same notation in set theory:\\
        \begin{itemize}
    ...
    \begin{proof}
        ...
        This claim can be proved by induction on the dimension $d$.

        Induction base: $d = 1$. We have $1$ and $0$ each appear exactly $n - 1$ times. We see that 
        to make a $\zeroSumSeq{n}$, we need to have either $n$ 0s or $n$ 1s, which is impossible.

        Induction step.  Assume that the claim is valid for all $k \leq d$. 
        For the dimension $d+1$. We consider the first $d$ components (for a vector $v \in \IntegerP{n}^{d+1}$, I use the notation $\tilde{v}$ to denote the truncated). Now due to increased dimension by one.
        The number of occurence of the vector in the first $d$ components are $2(n-1)$. Given $n$ summands, if each $\tilde{v}$ appear less than $n$ times, then
        we can reduce to the $d-1$ case and affirm that the sum is not $0$ in the $d$ components and thus also not zero in $\IntegerP{n}^{d+1}$. If there is $\tilde{v}$ appear more than $n-1$ times,
        so it could only be possible that all the $n$ summands have the same first $d$ components. But now it falls back to the induction basis, because we do not need to consider
        the first $d$ component and only focus on the $d+1$ component. We have at most $n-1$ 1s and $0$s which is impossible to have a zero sum.
    ...
    \begin{proof}
        ...
        According to the definition, each such subsequence $A_l$ sums to a vector $w_l$ of the form
    ...
    A natural question emerges that if this is the case for all $n \in \NaturalNumber$, that is
    \[\fnd{n}{2} = 4n - 3\]
...
\newpage
    ...
    We use the notation $\numSumSubset{m}{J}$ to denote the number of the \zeroSumSeq{m} (w.r.t $G$) in $J$.
    ...
    \begin{proof}
        ...
        \begin{case}
            ...
            \begin{enumeratei}
                ...
                This corresponds to the case where there needs to be $p-1$ elements in $J$ which form a zero-sum.
                There are in all 
    ...
    \begin{remark}
        ...
        all have the same number of monomials, i.e. $\abs{J}$, different from in the case in $\abs{J} = 3p-3$. This
    ...
    \begin{proof}
        ...
        We consider the sum:
        \[ \sum_{ \substack{Y \subset X, \abs[]{Y} = p, \\\sum_{y\in Y} y= 0}} \sum_{\substack{I,\sothat \\Y\subset I,\\\abs{I} = 3p-3}} 1\]
        Note that
        \begin{align*}
            \binom{3p-3}{2p-3} \numSumSubset{p}{X} 
            &= \sum_{ \substack{Y\subset X, \abs[]{Y} = p, \\\ \sum_{y\in Y} y = 0}} \binom{3p-3}{2p-3} &&= \sum_{ \substack{Y\subset X, \abs[]{Y} = p, \\\sum_{y\in Y} y= 0}} \sum_{\substack{I,\sothat \\Y\subset I,\\\abs[]{I} = 3p-3}} 1\\
                &=\sum_{\substack{I,\sothat\\\abs[]{I} = 3p-3}}\;\sum_{ \substack{Y \subset I,\abs[]{Y} = p,\\\sum_{y\in Y} y= 0}}  1
                &&= \sum_{I} \numSumSubset{p}{I}
        \end{align*}
...
\[        \begin{pmatrix}
    ...
    \begin{proof}
        ...
        \[v_{\chi} := \begin{pmatrix}
            ...
            \chi(g_n)
        \end{pmatrix},\]
        is dinstict for different $\chi \in \characterGroup$. Since $\adjacencyMatrixCayley{G}{S}$ is symmetric,
